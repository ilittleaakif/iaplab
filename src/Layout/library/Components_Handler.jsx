import { CalendarRange, ArrowRight, Layers, Youtube, Grid2X2Plus, ListVideo } from "lucide-react";
...
export const ExtraCard = ({ title, icon: Icon, Link }) => {
  return (
    <div
      ...
      className="text-accent gap-3 border-2 border-accent p-4  md:p-6 flex md:flex-col items-center md:justify-center hover:-translate-y-1 active:translate-y-1 cursor-pointer transition-all">
...
export const ModuleCard = ({ title, icon: Icon, Link }) => {
  return (
    <div
      onClick={() => OpenLink(Link)}
      className="text-text gap-3 border-2 border-text md:p-6 p-4 flex md:flex-col items-center md:justify-center hover:-translate-y-1 active:translate-y-1 cursor-pointer transition-all">
      <Icon className="w-5 h-5 md:w-8 md:h-8 " />
      <div className="font-bold text-center text-sm md:text-md">{title}</div>
    </div>
  );
};
...
export const YoutubeCard = ({ title, Link }) => {
  return (
    <div
      onClick={() => OpenLink(Link)}
      className="text-youtube gap-3 border-2 border-youtube md:p-6 p-4  flex md:flex-col items-center md:justify-center hover:-translate-y-1 active:translate-y-1 cursor-pointer transition-all">
...
export const SemesterContainer = ({ extras, youtube, modules, sectionRef }) => {
  return (
    <section ref={sectionRef} className="md:p-6 p-2 space-y-8 text-text">
